...
if (state === "NY") {
	...
	if (ageInput >= 18 && ageInput < 30 && wlInput >= 150000) {
      ...
      wlRate = NILrM[ageAdjusterWL];
      wlPremium = (wlInput / 1000) * (wlRate / 12);
		if (wlPremium < (51 / 12) && ageInput <= 23) {
			document.getElementById("wlInput" + plan + pi).value = formatFaceAmount(Math.floor(51000 / wlRate));
			wlPremium = 4.25;
		} else if (wlPremium < (103 / 12) && ageInput > 23) {
			document.getElementById("wlInput" + plan + pi).value = formatFaceAmount(Math.floor(103000 / wlRate));
			wlPremium = 8.59;
		}
		if (wlPremium > 43.21) {
